...
class CategoryDetailView extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _CategoryDetailViewState();
}

class _CategoryDetailViewState extends State<CategoryDetailView> with WidgetsBindingObserver {
  ...
  void initState() {
    super.initState();
    categoryViewModel.get(widget.id);
          ...
          IconButton(onPressed: () {
            showDialog(
              ...
              return Dialog(
                ...
                child: ConfirmPopUp(
                  onConfirm: () async {
                    await categoryViewModel.delete(widget.id).then((value) {
                      if (value) {
                        Utils.toastMessage("Categorie supprimee avec succes");
                        Navigator.of(parentContext).pushNamedAndRemoveUntil(
                            RoutesName.allCategories,
                                (route) => false
                        );
                      } else {
                        Utils.flushBarErrorMessage("Impossible de supprimer cette categorie", context);
                        Navigator.pop(parentContext);
                      }
                    });
                  },
                  message: "Etes-vous sure de vouloir suppriemer cette categorie ? Toutes les lecons de cette categorie seront egalement supprimees",
                ),
              );
            }
            );
      ...
      body: ChangeNotifierProvider<CategoryViewModel>(
        ...
        child: Consumer<CategoryViewModel>(
          builder: (context, value, _) {
            switch (value.categoryDetail.status)  {
              case Status.LOADING:
              ...
              case Status.ERROR:
                return Padding(
                  ...
                  child: Center(
                    child: Text(
                      value.categoryDetail.message.toString(),
              ...
              case Status.COMPLETED:
                CategoryModel category = value.categoryDetail.data!;
                return Column(
                    ...
                    Stack(children: <Widget>[
                      Container(
                        decoration: BoxDecoration(
                          ...
                          image: category.imagePath == null ? null : DecorationImage(
                            fit: BoxFit.cover,
                            image: NetworkImage(AppUrl.domainName + category.imagePath.toString()),
                          ),
                        ),
                        height: 200.0,
                      ),
                          ...
                          Container(
                            ...
                            child: Center(
                              child: Column(
                                ...
                                children: [
                                  Flexible(
                                    child: Text(category.name.toString(), style: const TextStyle(
                                        fontSize: 20,
                          ...
                          Expanded(
                            child: ListView.builder(
                              ...
                              itemBuilder: (context , index) {
                                LessonModel current = category.lessons![index];
                                return ListTile(
                                  onTap: () {
                                    Navigator.pushNamed(
                                        context,
